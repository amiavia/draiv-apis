"""
BMW API Simple - Docker Workaround + bimmer_connected
====================================================
Uses proven Docker workaround approach with bimmer_connected library.
Applies Android fingerprint patch then uses standard bimmer_connected authentication.

Based on main_stateless.py foundation but simplified:
- Android patch applied first (Docker workaround approach)
- Standard bimmer_connected authentication (no manual OAuth PKCE)
- Each deployment gets unique android() fingerprint
"""
...
async def authenticate_bmw_simple(email: str, password: str, hcaptcha_token: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    try:
        print(f"🔐 Simple authentication for {email}...")
...
def bmw_api_simple(request):
    """
    BMW API Simple - Docker workaround approach with bimmer_connected
    """
    ...
    print(f"🚗 Processing BMW API Simple request...")
    print(f"📧 Email: {email}")
    ...
    try:
        async def process_request():
            ...
            if action == "status":
                ...
                if not wkn:
                    return {
                        "error": "WKN (VIN) required for remote services"
                    }, 400
...
if __name__ == "__main__":
    print("BMW API Simple - Local Test Mode")
    print("=" * 50)
    ...
    print("\nReady for testing!")
    print("Use curl to test the API endpoints")